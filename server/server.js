...
app.get('/requestCount', (req, res) => {
    const sql = "Select count(id) as request from requester";
    con.query(sql, (err, result) => {
        if(err) return res.json({Error: "Error in running query"});
        return res.json(result);
    })
...
app.get('/getRequest', (req, res) => {
    const sql = "SELECT * FROM requester";
    con.query(sql, (err, result) => {
        if(err) return res.json({Error: "Get requester error in sql"});
        return res.json({Status: "Success", Result: result})
    })
})
...
app.post('/createreq',upload.single('requester_image'), (req, res) => {
    const sql = "INSERT INTO requester (`requester_name`,`requester_phoneNo`,`requester_email`,`requester_address`,`request`,`message`,`requester_image`) VALUES (?)";
    const values = [
        req.body.requester_name,
        req.body.requester_phoneNo,
        req.body.requester_email,
        req.body.requester_address,
        req.body.request,
        req.body.message,
        req.file.filename
    ]
    con.query(sql, [values], (err, result) => {
        if (err) return res.json({Error: "Inside requsting query"});
        return res.json({Status: "Success"});
    })
})
...
app.delete('/deletereq/:id', (req, res) => {
    const id = req.params.id;
    const sql = "Delete FROM requester WHERE id = ?";
    con.query(sql, [id], (err, result) => {
        if(err) return res.json({Error: "delete requester error in sql"});
        return res.json({Status: "Success"})
    })
})
